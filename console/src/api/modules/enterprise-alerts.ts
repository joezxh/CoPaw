...
export const getAlertRules = async (params?: { is_active?: boolean }) => {
  ...
  return request<AlertRule[]>(`/api/enterprise/alerts/rules?${qs.toString()}`);
};

export const createAlertRule = async (data: Omit<AlertRule, "id" | "created_at">) => {
  return request<AlertRule>("/api/enterprise/alerts/rules", {
    method: "POST",
...
export const updateAlertRule = async (id: string, data: Partial<Omit<AlertRule, "id" | "rule_type" | "created_at">>) => {
  return request<AlertRule>(`/api/enterprise/alerts/rules/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
  });
};
...
export const deleteAlertRule = async (id: string) => {
  return request<{ detail: string }>(`/api/enterprise/alerts/rules/${id}`, {
    method: "DELETE",
  });
};
...
export const getAlertEvents = async (params?: { rule_type?: string; offset?: number; limit?: number }) => {
  ...
  return request<{ total: number; items: AlertEvent[] }>(`/api/enterprise/alerts/events?${qs.toString()}`);
};

export const testNotification = async (message: string) => {
  return request<{ detail: string }>("/api/enterprise/alerts/test", {
    method: "POST",
