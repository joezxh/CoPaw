...
## 更新摘要
**变更内容**
- 更新数据库架构说明，反映 embedding 列从 Text 类型改为 pgvector 原生 vector 类型的重大技术升级
- 修正向量存储和检索的技术细节，确保与实际数据库架构一致
- 更新性能优化和故障排除指南，反映新的向量索引和存储机制
- 新增 1536 维向量操作和相似性搜索优化的相关内容
...
**重大技术升级**：数据库架构已升级为使用 pgvector 扩展的原生 vector 类型，提供更高效的向量存储和检索能力。新的架构支持 1536 维向量操作，使用 IVFFlat 索引提供高效的余弦距离相似性搜索。
...
subgraph "内存管理层"
F[BaseMemoryManager]
G[ReMeLightMemoryManager]
H[ReMePostgresBackend]
end
subgraph "存储层"
I[本地文件系统]
J[SQLite/ChromaDB]
...
**1536维向量优化**:
- **向量类型**: 使用原生 `vector(1536)` 类型提供精确的 1536 维向量存储
- **相似性搜索**: 通过余弦距离计算实现高效的语义匹配
- **索引优化**: IVFFlat 索引支持大规模向量数据的快速检索
...
**5. pgvector 扩展问题**
- **症状**: 数据库连接时报错，提示缺少 vector 扩展
- **原因**: PostgreSQL 未启用 pgvector 扩展
- **解决**: 确保 PostgreSQL 服务器启用了 vector 扩展，并且具有适当的权限

**6. 向量维度不匹配**
- **症状**: 向量搜索报错或结果异常
- **原因**: 嵌入模型维度与数据库向量类型不匹配
- **解决**: 确保嵌入模型输出维度与数据库 `vector(1536)` 类型一致
...
**重大技术升级**：
- **数据库架构升级**: 从 Text 类型的 embedding 列升级为 pgvector 原生 vector 类型
- **性能提升**: 新的向量存储格式提供更好的存储效率和检索性能
- **索引优化**: 支持更高效的 IVFFlat 向量索引和余弦距离计算
- **1536维向量支持**: 完全支持 1536 维向量操作，提供更高的语义表达能力
- **企业级优化**: 专为大规模企业部署优化，支持多租户和高并发访问
