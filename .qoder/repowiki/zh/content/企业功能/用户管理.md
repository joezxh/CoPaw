...
- [console/src/api/modules/enterprise-orgs.ts](file://console/src/api/modules/enterprise-orgs.ts)
</cite>

## 更新摘要
**所做更改**
- 新增组织管理章节，详细介绍基于部门的用户组织结构
- 更新用户模型，反映从用户组管理到组织管理的转变
- 新增部门层级结构和权限分配机制
- 更新角色模型，支持部门级别的角色管理
- 新增组织管理API接口文档
- 更新架构图和流程图以反映新的组织管理模式
...
## 简介
本文件面向 CoPaw 企业版的用户管理能力，围绕用户注册、登录认证、密码管理、账户状态控制、角色与权限继承、组织管理、批量导入导出（概念性）、用户生命周期管理、账户锁定/解锁、密码策略配置等企业级需求，提供实现原理、操作流程与最佳实践。内容基于后端服务与数据库模型，辅以可视化图示帮助非技术读者理解。

**更新** 本版本重点反映了用户管理功能从传统的用户组管理向现代组织管理的演进，引入了基于部门的层级化组织结构和权限分配机制。

## 项目结构
与用户管理直接相关的代码主要分布在以下模块：
- 数据模型层：用户、角色、会话、权限、部门组织结构等
- 路由层：用户管理 API、认证 API、部门管理 API
...
RAuth["/auth<br/>认证路由"]
RDepartments["/api/enterprise/departments<br/>部门管理路由"]
end
subgraph "企业服务层"
...
- [src/copaw/app/routers/departments.py:1-323](file://src/copaw/app/routers/departments.py#L1-323)
- [src/copaw/enterprise/auth_service.py:1-367](file://src/copaw/enterprise/auth_service.py#L1-367)
- [src/copaw/enterprise/rbac_service.py:1-322](file://src/copaw/enterprise/rbac_service.py#L1-322)
- [src/copaw/enterprise/middleware.py:1-191](file://src/copaw/enterprise/middleware.py#L1-191)
- [src/copaw/db/models/user.py:1-92](file://src/copaw/db/models/user.py#L1-92)
- [src/copaw/db/models/role.py:1-150](file://src/copaw/db/models/role.py#L1-150)
- [src/copaw/db/models/organization.py:1-88](file://src/copaw/db/models/organization.py#L1-88)
...
## 核心组件
- 用户模型与组织结构
  - 用户实体包含用户名、邮箱、密码哈希与盐、显示名、部门ID、状态、MFA 开关与密钥、最后登录时间等字段，并维护与角色、会话、部门的关联关系。
  - 部门模型支持层级化的组织结构，使用邻接表模式实现树形结构，支持递归 CTE 查询完整的祖先/后代链条。
- 角色与权限
  - 角色支持最多 5 级的父子继承链，具备层级 level 字段；角色与权限通过关联表建立多对多关系；用户与角色通过关联表建立多对多关系并记录分配者与时间。
  - 新增部门级别的角色管理，支持按部门维度分配角色。
- 会话与令牌
...
**章节来源**
- [src/copaw/db/models/user.py:25-92](file://src/copaw/db/models/user.py#L25-92)
- [src/copaw/db/models/role.py:24-150](file://src/copaw/db/models/role.py#L24-150)
- [src/copaw/db/models/organization.py:21-88](file://src/copaw/db/models/organization.py#L21-88)
...
- [src/copaw/enterprise/rbac_service.py:30-322](file://src/copaw/enterprise/rbac_service.py#L30-322)
...
## 架构总览
下图展示用户管理的关键交互路径：注册/登录由认证服务处理，用户管理路由调用认证与审计服务，RBAC 服务负责权限判定，会话表支撑令牌撤销与审计，部门管理提供组织结构支撑。

```mermaid
sequenceDiagram
participant 客户端 as "客户端"
participant 路由 as "用户管理路由"
participant 部门路由 as "部门管理路由"
...
客户端->>部门路由 : "POST /api/enterprise/departments"
部门路由->>数据库 : "创建部门记录(层级结构)"
部门路由->>审计服务 : "记录部门创建审计"
客户端->>路由 : "POST /api/enterprise/users"
路由->>认证服务 : "注册(用户名/密码/邮箱/显示名)"
认证服务->>数据库 : "写入用户记录(关联部门ID)"
认证服务-->>路由 : "返回新用户"
路由->>审计服务 : "记录用户创建审计"
路由-->>客户端 : "返回用户信息"
客户端->>路由 : "POST /api/enterprise/users/{id}/roles"
路由->>RBAC服务 : "清空旧角色并分配新角色(支持部门级别)"
RBAC服务->>数据库 : "写入用户角色关联"
路由->>审计服务 : "记录角色分配审计"
路由-->>客户端 : "返回成功"
```
...
**图表来源**
- [src/copaw/app/routers/departments.py:116-148](file://src/copaw/app/routers/departments.py#L116-148)
...
### 用户模型与组织结构
- 用户实体
  - 关键字段：用户名唯一、邮箱唯一、密码哈希与盐、显示名、部门ID外键、状态（active/disabled/vacation）、MFA 开关与密钥（加密存储）、最后登录时间。
  - 关系：与部门、角色、会话关联，支持通过部门ID进行组织结构查询。
- 部门模型
  - 支持层级化的组织结构，使用邻接表模式实现树形结构，parent_id 指向父部门，level 表示层级深度。
  - 支持递归 CTE 查询完整的祖先/后代链条，便于组织结构遍历和权限计算。
  - 新增 manager_id 字段，支持部门负责人管理。
- 复杂度与性能
...
LoadPerms --> CheckDeptScope["检查部门权限范围"]
CheckDeptScope --> BuildSet["合并去重得到权限集"]
...
### 登录认证与会话管理
- 注册
  - 校验用户名与邮箱唯一性，生成密码哈希与盐，创建用户记录（可选择关联部门）。
- 登录
...
### 组织管理 API 流程
- 组织树查询
  - 支持获取完整的部门层级树结构，使用递归 CTE 查询所有子部门。
- 创建部门
  - 支持指定父部门、负责人、描述等信息，自动计算层级深度。
- 更新部门
  - 支持修改部门名称、父部门、负责人、描述等属性。
- 删除部门
  - 不允许删除仍有子部门的部门，防止组织结构断裂。
- 成员管理
  - 支持批量添加用户到部门、移除部门成员、查询部门成员列表。
- 统计信息
  - 提供部门成员数量、子部门数量等统计信息。
...
participant 部门路由 as "部门管理路由"
participant 数据库 as "PostgreSQL"
管理员->>部门路由 : "POST /api/enterprise/departments"
部门路由->>数据库 : "创建部门记录(设置层级)"
部门路由-->>管理员 : "返回部门信息"
管理员->>部门路由 : "POST /api/enterprise/departments/{id}/members"
部门路由->>数据库 : "批量更新用户部门ID"
部门路由-->>管理员 : "返回添加结果"
```
...
**图表来源**
- [src/copaw/app/routers/departments.py:116-148](file://src/copaw/app/routers/departments.py#L116-148)
- [src/copaw/app/routers/departments.py:239-269](file://src/copaw/app/routers/departments.py#L239-269)

**章节来源**
- [src/copaw/app/routers/departments.py:76-323](file://src/copaw/app/routers/departments.py#L76-323)
- [console/src/api/modules/enterprise-orgs.ts:36-104](file://console/src/api/modules/enterprise-orgs.ts#L36-104)
...
- 审计集成
  - 路由层在关键操作（创建用户、更新用户、禁用用户、分配角色、部门管理）后记录审计事件，包含资源类型、前后数据、操作者等。
- DLP 扫描
...
- 路由依赖
  ...
  - 部门管理路由依赖认证服务、RBAC 服务、审计服务。
  - 认证路由依赖认证服务与数据库状态检查。
- 服务依赖
  - 认证服务依赖用户与会话模型；RBAC 服务依赖角色、权限、用户、部门模型；加密工具被用户与会话模型使用。
- 中间件依赖
...
- [src/copaw/app/routers/departments.py:26](file://src/copaw/app/routers/departments.py#L26)
...
- [src/copaw/db/models/user.py:17-92](file://src/copaw/db/models/user.py#L17-92)
...
- [src/copaw/db/models/organization.py:15-88](file://src/copaw/db/models/organization.py#L15-88)
...
- 查询与索引
  ...
  - 部门树查询使用递归 CTE，建议在 parent_id 字段上保持索引以提升层级查询性能。
- 缓存
...
- 部门管理异常
  - 检查部门层级关系是否正确；确认父部门存在且层级不超过限制；验证部门删除时无子部门。
- MFA 登录异常
...
- [src/copaw/app/routers/departments.py:192-216](file://src/copaw/app/routers/departments.py#L192-216)

## 结论
CoPaw 企业版的用户管理以清晰的数据模型与服务层设计为基础，实现了从用户注册、登录认证、密码与 MFA 管理，到角色权限继承、会话撤销与审计追踪的完整闭环。**更新** 新版本引入了基于部门的层级化组织结构，支持更精细的权限管理和组织治理。通过合理的索引、缓存与加密策略，可在保证安全的前提下满足企业级大规模并发与合规要求。建议结合组织实际，完善密码策略、审计范围与 DLP 策略，持续优化用户体验与运维效率。
...
- 组织管理最佳实践
  - 建议按照实际业务部门结构设计组织层级，避免过深的层级关系影响性能；
  - 合理规划角色权限，遵循最小权限原则；
  - 定期审查部门成员关系和角色分配，确保权限的时效性和准确性。